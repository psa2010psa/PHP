$(function(){
    ...
    try{
        $(".chkbox2").iButton({className: "enabled_disabled", labelOn: "选择", labelOff: "取消", easing: "swing"});
    } catch (e){
        errorMessage(e);
    }
...
$('.menuDrop').on('click', function(){
    if($('.search > .search_form').hasClass('visible')){
...
$(document).mouseup(function(e) {
    if(jQuery(e.target).parents().index() < 4 || jQuery(e.target).parents().index() > 6){
        $('.search_form').removeClass('visible');
        $('.search').hide();
    }
});
